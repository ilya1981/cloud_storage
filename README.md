...
Внутри консоли PostgreSQL:
        ...
        CREATE USER user WITH PASSWORD 'ваш_сложный_пароль';
...
- Откройте в браузере: http://<IP_ИЛИ_ДОМЕН>/ — должна загрузиться React-страница.
= Проверьте API:
...
#### Проект развёрнут на VPS, потому что облачное хранилище требует:
   - Полного контроля над процессами (Gunicorn, воркеры).
   - Прямой работы с файловой системой и правами доступа.
   - Надёжного соединения с PostgreSQL.
   - Автоматизации через CI/CD и systemd.
   - Такой подход обеспечивает масштабируемость, безопасность и предсказуемость работы сервиса.


### Контакты:
  - автор: ilya1981
  - mail: ilya110881@gmail.com
